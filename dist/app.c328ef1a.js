...
var baseURL = "https://swapi.dev/api/vehicles/?pages="; // functions
...
var renderHTML = function renderHTML(swVehicles) {
  var _iterator = _createForOfIteratorHelper(swVehicles),
      _step;

  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      var vehicle = _step.value;
      var vehicleHTML = "<div data-name=".concat(vehicle.name, " class=\"container__vehicle container__vehicle--1\">\n    <p class=\"container__vehicle-name\">\n      <span>Name:</span> ").concat(vehicle.name, "\n    </p>\n    <p class=\"container__vehicle-model\">\n      <span>Model:</span> ").concat(vehicle.model, "\n    </p>\n    <p class=\"container__vehicle-manufacturer\">\n      <span>Manufacturer:</span> ").concat(vehicle.manufacturer, "\n    </p>\n    <p class=\"container__vehicle-cargo\">\n      <span>Cargo-capacity:</span> ").concat(vehicle.cargo_capacity, "\n    </p>\n    <p class=\"container__vehicle-url\">\n      ").concat(vehicle.url, "\n    </p>\n  </div>"); // console.log(vehicle);
...
var renderSingleHTML = function renderSingleHTML(vehicle) {
  var vehicleHTML = "<div data-name=".concat(vehicle.name, " class=\"container__vehicle container__vehicle--1\">\n    <p class=\"container__vehicle-name\">\n      <span>Name:</span> ").concat(vehicle.name, "\n    </p>\n    <p class=\"container__vehicle-model\">\n      <span>Model:</span> ").concat(vehicle.model, "\n    </p>\n    <p class=\"container__vehicle-manufacturer\">\n      <span>Manufacturer:</span> ").concat(vehicle.manufacturer, "\n    </p>\n    <p class=\"container__vehicle-cargo\">\n      <span>Cargo-capacity:</span> ").concat(vehicle.cargo_capacity, "\n    </p>\n    <p class=\"container__vehicle-url\">\n      ").concat(vehicle.url, "\n    </p>\n  </div>"); // console.log(vehicle);
...
function fetchSWVehicles() {
  ...
  // }
  var swPromise = fetch(baseURL).then(function (res) {
    console.log(res);
    return res.json();
  }).then(function (res) {
    var constructedData = [];
    var swVehicles = res.results; // console.log('Something', swVehicles);

    var _iterator2 = _createForOfIteratorHelper(swVehicles),
        _step2;
...
var fetchSWSingleVehicle = function fetchSWSingleVehicle(vehicleName, cb) {
  var indiSection = find('#individual');
  loader.style.display = 'block'; // console.log(vehicleName.url.length);
  ...
  fetch("".concat(vehicleName.url)) // fetch(`http://swapi.dev/api/vehicles/7/`)
...
var checkForHash = function checkForHash() {
  if (window.location.hash !== '#home' && window.location.hash !== '#individual') {
    window.location.hash = '#home';
  }
};

var renderPage = function renderPage(hashValue) {
  // section visible
  find(hashValue).style.display = 'block';

  if (hashValue === '#home') {
    fetchSWVehicles().then(function (vehicles) {
...
var init = function init() {
  checkForHash(); // rendering that page alone
...
window.addEventListener('hashchange', function (event) {
  ...
  checkForHash(); // resetting the style property

  find('#home').style.display = 'none';
  find('#individual').style.display = 'none'; // rendering that page alone

  renderPage(window.location.hash);
}); // To select a individual card,

vehicleContainer.addEventListener('click', function () {
  var isCardClicked = event.target.matches('.container__vehicle, .container__vehicle *'); // If the selection is inside a card,

  if (isCardClicked) {
    var containerCard = event.target.closest('.container__vehicle'); // console.log('Akash', containerCard);
    ...
    var result;
    fetchSWSingleVehicle(vehicleName, function (data) {
      result = data;
      console.log(result);
